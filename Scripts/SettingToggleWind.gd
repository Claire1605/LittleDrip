...
@export var thisWindUnit: String

func _ready() -> void:
	if get_node_or_null(weatherRequest).saveData.windUnit == thisWindUnit:
...
func SelectSetting(node):
	get_node_or_null(weatherRequest).saveData.saveWindUnit(thisWindUnit)
	node.disabled = true
